    ...
    try {
        ...
        if(existingUser){
         ...
         if(password.length<8){
            return res.json({success:false,message:"Password need at least 8 Charater"})
         }
         // hashed password
   ...
   try {
     ...
     if(!password) return res.json ({success:false,message:"Password is Required"})
       ...
       if(isMatch){
        ...
        res.json({success:true,token:token,message:"User Login Succesfully"})
       }
       else{
...
const getUsers=async(req,res)=>{
    res.send("Jillur User Get Succesfully")
}

export {userRegister,userLogin,adminLogin,removeUser,updateUser,getUsers};